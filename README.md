# Sanchay — savings group app

A web app for friends' savings groups (*sangam*, *chit*, *committee*): monthly
contributions, loans with group voting, expenses, the cashier's cash float, bank
reconciliation, and an audit log nobody can edit.
...
becomes its president; everyone else joins with an invite code and is then
approved by an officer. Each group's money, members and rules are its own, and
...
1. Sign up at [supabase.com](https://supabase.com) and create a project (the
   free tier is ample for 7 members).
...
design, because the audit log cannot be deleted from.
...
It also checks that writes cannot cross (voting on another group's loan,
approving another group's member) and that a *pending* member sees nothing at
all. It ends in `ROLLBACK`, so it leaves nothing behind. The last line under
**Messages** should read `ISOLATION: all 43 checks passed`.
...
both have free tiers well above what seven people need.
...
3. You are now its president, which is what lets you invite people and set the
   rules.
...
8. On **Members → Offices**, hand the cashier and accountant roles to the right
...
single row. An officer has to approve them first. Codes get forwarded into
WhatsApp groups, and one forward should not expose a ledger.
...
| Rule | Enforced by |
|---|---|
| Nobody votes on their own loan | `cast_loan_vote` RPC; `loan_votes` has no INSERT policy |
| 4 approvals needed | `cast_loan_vote`, with the loan row locked `FOR UPDATE` |
| Max 30% of the fund per member | Checked in `request_loan` against **aggregate** outstanding |
| 25% reserve never lent | Re-checked at request, at approval, and again at disbursal |
...
| Cash float ≤ ₹5,000 | Statement-level trigger on `cash_ledger` |
| Late fee after the grace date | Applied server-side in `record_contribution` |
| Group expenses ≤ 20% of fund per year | Checked in `cast_expense_vote` at approval |
...
| The group can be claimed only once | `claim_group` locks `app_config`, then checks `claimed_at` |
| A login is matched to a member by email | `link_signed_in_member` reads the address from `auth.users`, which the client cannot forge |
| A member leaving keeps their history | `remove_member` sets `left_on`; it never deletes, and refuses while they owe money |

Two properties worth knowing about:

**Concurrency.** Votes and approvals take `app_config FOR UPDATE` and then the
loan row `FOR UPDATE`, always in that order. Without this, two members voting at
the same moment could each see three approvals and each add a fourth, and two
loans approved simultaneously could jointly break the reserve. The lock order is
fixed so two sessions cannot deadlock.
...
- **Total fund** = contributions + interest *received* − expenses. Money out on
  loan is still an asset, so it is not subtracted.
- **Expected bank balance** = total fund − loans outstanding − cash float. This
  is the number that must equal the bank statement.
...
| When | Who | What |
|---|---|---|
| 1st–5th | everyone | Pay the contribution |
| 5th | cashier | Post the bank statement screenshot in the group |
| 6th–9th | accountant | Record contributions, repayments and expenses |
| 10th | accountant | Record the bank statement on the **Bank** page and confirm the difference is **zero** |
| monthly | everyone | 30-minute meeting |
| every 6 months | 2 random members | Audit — compare the **Audit** page against the bank statement |
...
1. `supabase/migrations/0012_multi_group.sql` — tenancy: every policy, every
   ...
   money, silently.
2. `supabase/migrations/0009_loan_voting.sql` — the voting rule and the locking
